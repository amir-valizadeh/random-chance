...
const SlotMachine: React.FC<SlotMachineProps> = ({ 
    ...
    useEffect(() => {
        if (isSpinning && participants.length > 0) {
            ...
            let selectedWinner: Participant | null = null;
            let weightSum = 0;
            ...
            if (selectedWinner && extendedList.length > 0) {
                ...
                const winnerIdx = extendedList.findIndex((p, i) => i >= midPoint && p.id === selectedWinner!.id);
                const targetIdx = winnerIdx >= 0 ? winnerIdx : midPoint;

                // Calculate scroll distance
                const targetOffset = -(targetIdx - Math.floor(VISIBLE_ITEMS / 2)) * ITEM_HEIGHT;
    ...
    }, [isSpinning, participants, totalWeight, extendedList, controls, setIsSpinning, onSpinEnd]);
        ...
        <div className="relative w-full max-w-[500px] mx-auto">
            ...
            <div className="bg-gradient-to-b from-gray-700 via-gray-800 to-gray-900 p-6 rounded-3xl shadow-2xl border-4 border-gray-600">
                    ...
                    <div className="flex gap-2">
                        {[...Array(9)].map((_, i) => (
                            <div 
                                ...
                                style={{ 
                                    backgroundColor: ['#FF6B6B', '#FFE66D', '#4ECDC4', '#9B59B6', '#FF6B6B', '#9B59B6', '#4ECDC4', '#FFE66D', '#FF6B6B'][i],
                ...
                >
                    ...
                    >
                        {/* Side arrows */}
                        <div className="absolute left-2 top-1/2 -translate-y-1/2 text-yellow-400 text-2xl">▶</div>
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 text-yellow-400 text-2xl">◀</div>
                    </div>
                    ...
                    >
                        {displayItems.map((p, idx) => (
                            <div
                                ...
                                dir={isRTL ? 'rtl' : 'ltr'}
                            >
                                <div className={`flex items-center gap-4 w-full max-w-[350px] ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                                    <div 
                                    ...
                                    <span 
                                        ...
                                        style={{ 
                                            fontFamily: isRTL ? "'Vazirmatn', system-ui, sans-serif" : "system-ui, sans-serif",
