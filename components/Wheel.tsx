...
const Wheel: React.FC<WheelProps> = ({ participants, onSpinEnd, isSpinning, setIsSpinning }) => {
    ...
    const segments = useMemo(() => {
        ...
        return participants.map((p) => {
            const angleSize = (p.weight / totalWeight) * 360;
            const segment = {
                participant: p,
    ...
    useEffect(() => {
        ...
        if (showNames) {
            ...
            segments.forEach((seg) => {
                ...
                const nameLength = seg.participant.name.length;
                ...
                // Truncate long names
                let displayName = seg.participant.name;
    ...
    const spinWheel = useCallback(async () => {
        ...
        // Calculate random winner based on weights
        let randomWeight = Math.random() * totalWeight;
        let winnerIndex = -1;
        let weightSum = 0;

        for (let i = 0; i < participants.length; i++) {
            weightSum += participants[i].weight;
        ...
        const targetRotation = rotation + (360 * extraSpins) + (360 - winnerCenterAngle - currentRotationMod);
        ...
        setIsSpinning(false);
        onSpinEnd(winner);
    }, [participants, totalWeight, segments, rotation, controls, setIsSpinning, onSpinEnd]);
